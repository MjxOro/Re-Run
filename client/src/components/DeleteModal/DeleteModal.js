...
const DeleteModal = (props) =>{
	const card = props.data.find(elem =>{return elem._id === props.postId })
	console.log(card)
	return(
		<section className={props.show ?'delete':'delete delete--close'}>
			<div className={props.show ?'delete__container':'delete__container delete__container--close'}>
				<Link to='/' onClick={props.close} className='delete__close' />
				<h1 className='delete__greeting'>Are you sure you want to delete this posting?</h1>
			{ card && (
				...
				<div className='delete__icon-container'>
					<div onClick={props.handleDelete} className='delete__delete-btn delete__delete-btn--container'>
						<ImCheckmark className='delete__delete-btn'/>
					</div>
					<div onClick={props.close} className='delete__close-btn delete__close-btn--container'>
